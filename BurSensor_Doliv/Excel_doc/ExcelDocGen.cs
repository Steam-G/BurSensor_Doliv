...
namespace BurSensor_Doliv.Excel_doc
{
    class ExcelDocGen
    {
        ...
        public void GenerateExeleDoc()
        {
            ...
            try
            {
                ...
                //Заполнение 
                CellHeadVal(oSheet, 3, 3, "C3", "E3", _ListInfoReis.ValMestorojdenieStr); // Месторождение
                CellHeadVal(oSheet, 7, 3, "G3", _ListInfoReis.ValKustStr); // Куст №
                CellHeadVal(oSheet, 9, 3, "I3", _ListInfoReis.ValSkvajinaStr); // Скв. №
                CellHeadVal(oSheet, 11, 3, "K3", Date.ToString("d", CultureInfo.CreateSpecificCulture("de-DE"))); // Дата:
                CellHeadVal(oSheet, 3, 4, "C4", _ListInfoReis.ValBrigadaStr); // Бригада №
                CellHeadVal(oSheet, 6, 4, "F4", "J4", _ListInfoReis.ValBurilshikStr);   // Бурильщики
                CellHeadVal(oSheet, 7, 5, "G5", "M5", _ListInfoReis.ValOtvZaZapolnenieListaDolivaStr);   // Ответственные за заполнение листа долива:
                CellHeadVal(oSheet, 8, 6, "H6", "M6", _ListInfoReis.ValOtvZaUchetKolichestvaBIStr);   // Ответственные за учет кол-ва поднятого/спущенного БИ:
                CellHeadVal(oSheet, 4, 7, "D7", _ListInfoReis.ValZaboiStr); // Забой скважины: 
                CellHeadVal(oSheet, 7, 7, "G7", "M7", _ListInfoReis.ValPrichinaSPOStr);   // Причина/Цель СПО:
                CellHeadVal(oSheet, 4, 8, "D8", _ListInfoReis.ValPlotnostBRStr); // Плотность БР:
                CellHeadVal(oSheet, 8, 8, "H8", "I8", _ListInfoReis.ValTimeStartSPOStr); // Время начала СПО:
                ...
                string[,] stArrDoliv = GetArrStringListDoliva();
                FillTable(oSheet, stArrDoliv);
                ...
                int InfoTabStartRow = 12 + stArrDoliv.GetLength(0); // 12 - строка начала данных, плюсуем количество строк данных
                string[,] stArrInfoKNBK = GetArrStringInfoKNBK();
        ...
        private string[,] GetArrStringInfoKNBK()
        {
            string[,] _arrInfoKNBK = new string[_ListKNBK.Count, 3];
            int i = 0;
            foreach (var item in _ListKNBK)
            {
                _arrInfoKNBK[i, 0] = item.TypeKNBK;
                _arrInfoKNBK[i, 1] = item.V1.ToString();
                _arrInfoKNBK[i, 2] = item.V2.ToString();
        ...
        private string[,] GetArrStringListDoliva()
        {
            string[,] _listDoliva = new string[_ListDoliva.Count, 10];
            int i = 0;
            foreach (var item in _ListDoliva)
            {
                _listDoliva[i, 0] = item.TypeKNBK;
                _listDoliva[i, 1] = item.SvechaCapacity.ToString();
                _listDoliva[i, 2] = item.MeraBurInstrument.ToString("#.##");
                _listDoliva[i, 3] = item.ObyemJidkostiDoliv.ToString("#.##");
                _listDoliva[i, 4] = item.Raschet.ToString("#.##");
                _listDoliva[i, 5] = item.RaschetSum.ToString("#.##");
                _listDoliva[i, 6] = item.Fact.ToString("#.##");
                _listDoliva[i, 7] = item.FactSum.ToString("#.##");
                _listDoliva[i, 8] = item.SumRaznDoliv.ToString("#.##");
                _listDoliva[i, 9] = item.Primechanie;
                i++;
            }
            return _listDoliva;
        }
